...
# ----------------------------------------------------------------
def main():
    st.title("Warehouse Planning Tool")
    ...
    st.info("These sliders are pre-set to industry standard allocations for warehouse budgeting")
    if 'slider_values' not in st.session_state:
    # Set industry standard defaults instead of equal distribution
        st.session_state.slider_values = {
        "Storage System": 50.0,
        "Material Handling Equipment": 30.0,
        "Dock Equipment": 10.0,
        "Packing and Sorting Systems": 10.0
    }
            ...
            if st.button("Generate Recommendations"):
                if abs(sum(budget_allocations.values()) - 100) > 1e-6:
                    ...
                    for eq_type, group in grouped:
                        ...
                        # Calculate average unit cost correctly
                        # For items with quantity > 1, we need to divide by quantity first
                        ...
                        for _, item in group.iterrows():
                            ...
                            if quantity > 1:
                                # For aggregated items, divide total cost by quantity to get unit cost
                                unit_cost = item["Unit Cost"] / quantity
                            else:
                                unit_cost = item["Unit Cost"]
                            actual_unit_costs.append(unit_cost)
                        
                        # Calculate average of actual unit costs
                        avg_cost = sum(actual_unit_costs) / len(actual_unit_costs) if actual_unit_costs else 0
                    ...
                    with colF3:
                        st.metric("Remaining", f"${rem_budget:,.0f}")

                    # Provide a subheader for breakdown details
                    st.subheader("Financial Breakdown")

                    # Build a pie chart showing Spent vs. Remaining
                    financial_data = pd.DataFrame({
                        "Category": ["Spent", "Remaining"],
                        "Value": [spent, rem_budget]
                    })
                    fig_financial = px.pie(financial_data, names="Category", values="Value")
